...
function extractSSLDetails(certificate) {
    if (!certificate) {
        ...
        return {'Exists': 0, 'Valid': 0, 'Issuer': "Unknown"};
    }
    ...
    return {'Exists': 1, 'Valid': validExpiryDate, 'Issuer': issuer};
}
...
function validateContentType(event) {
...
export const handler = async (event) => {
    try {
        ...
        console.log(`Fetched API results for ${finalDomain}`);
