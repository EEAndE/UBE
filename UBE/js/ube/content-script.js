...
const _executeScript = async (tabId) => {
    ...
    try {
        ...
        throw new Error(`executeScript failed: ${error.message}`, {cause: error});
    }

    if (injectionRes?.length === 0) {
        throw new Error('executeScript returned an empty array', {cause: error});
    }
};
...
export const UBEContentScript = {
    ...
    async triggerContentScriptCheck(tabId, url) {
        ...
        let response;
        const timeoutObj = _createTimeout('Content script start response timeout');
