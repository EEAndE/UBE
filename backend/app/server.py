...
def main():
    ...
    def server_stat():
        """
        endpoint: Simple health check indicating the service is up.
        Returns:
                int: "status": e.g 200,
                string: "model_version": e.g "v1",
                int: "urls_in_db": e.g  url_count,
                int: "updates_so_far": e.g self.updates_so_far
        """
    ...
    def get_api_key():
        """
        endpoint: Hand out a fresh API key.
        """
    ...
    def submit_new_phish_urls():
        """
       endpoint: Accept a batch of URLs from the extension.

        Requires:
        - Valid 'X-API-KEY'
        - JSON body: {"daily_urls": ["https://...", ...]}

        Adds them to today’s in-memory set and returns how many were accepted.
        """
        msg, status = dh.check_api_key(request)
        if status != OK:
            return dh.make_msg(error=msg), status
