...
DEFAULT_GH_PATH   = "blocklist/urls.txt"
...
class data_handler:
    ...
    def get_git_api_key():
        ...
        return os.environ.get("UBE_API_KEY") #must enter valid api key of the phishing list to  env var
    ...
    def insert_to_phish_db(self, scored_map):
        thr = getattr(self, "db_treshold",self.db_treshold)
        inserted = 0
    ...
    def validate_against_model(self, return_map: bool = False):
        ...
        # DEBUG
        dbg = str(os.environ.get("DEBUG", "")).strip().lower() in ("1")
        if dbg:
